...
def getjson(targets,name):
    with open("%s.json"%name,"w") as f:
            f.write(targets.encode('utf-8'))
...
def loadFont(target,name):
    sonata={'作者':" ",'年代':" ",'器乐类型':" ",'地域':" ",'资源路径':" ",'名称':" ",'旋律':" ",'URL':" ",'和声':" ",'语种':" ",'曲式':" ",'民族':" ",'声乐类型':" ",'版权':" ",'调式':" ",'复调':" ",'节奏':" ",'关键描述':" ",'类别':'music'}
    sonata['作者']=target['author']
    sonata['资源路径'] = target['link']
    sonata['名称'] = target['title']
    sonata['URL'] = target['url']
    sonata['语种'] = target['华语']
...
def download_mp3(downloadUrl,filepath):
    ...
    try:
        ...
        log_path = shellcode.communicate()
    ...
    log_path=str(log_path).split("\'")[1]
    if check_log(log_path) == True:
        # os.rename("temp.mp3", filepath)
        print "%s download Successful \n" % (filepath)
        return True
    else:
        print "%s download Fail,try again \n" % (filepath)
        return False

def download_song(key,infoid=0):
    songinfo = searching_page(key,infoid)
    # songinfos.append(songinfo)
    if download_mp3(songinfo[2], "%s.mp3" % key) == False:
        # infoid=infoid+1
        # print infoid
        download_song(key=key,infoid=(infoid+1))
    else:
        return
...
if __name__=="__main__":
    ...
    for key in keys:
        try:
            download_song(key,infoid=0)
            getjson(firstresult,key)
