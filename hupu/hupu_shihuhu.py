...
def open_url(url):
    ...
    headers = {
        ...
        "Referer": "https://bbs.hupu.com/vote-2",
...
def get_text(url,key):
    ...
    while(i<=key):
        ...
        result=BeautifulSoup(html.content, "lxml", from_encoding='utf-8').find_all(name=re.compile("^a"),class_="truetit")#此处使用正则表达式筛选页面信息

        for result_string in  result:
            count = count + 1
            result_table.append(str(str(count)+". "+result_string.string.encode('utf-8')))
            print str(str(count)+". "+result_string.string.encode('utf-8'))
